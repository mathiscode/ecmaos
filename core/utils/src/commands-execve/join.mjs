...
function main() {
  ...
  if (files.length !== 2) {
    writeAll(2, new TextEncoder().encode('join: exactly two files must be specified\n'))
    return 1
  }
  ...
  try {
    ...
    writeAll(2, new TextEncoder().encode(`join: ${message}\n`))
    return 1
  }
}
...
try {
  ...
  writeAll(2, new TextEncoder().encode(`join: ${error instanceof Error ? error.message : String(error)}\n`))
  exit(1)
}
