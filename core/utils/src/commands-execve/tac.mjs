...
function main() {
  ...
  for (const arg of args) {
    if (!arg.startsWith('-')) {
      ...
      write(2, new TextEncoder().encode(`tac: invalid option -- '${arg.slice(1)}'\n`))
      write(2, new TextEncoder().encode("Try 'tac --help' for more information.\n"))
      return 1
    }
  }
  ...
  if (files.length === 0) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        write(2, new TextEncoder().encode(`tac: ${file}: ${message}\n`))
        hasError = true
      }
    }
  }
...
try {
  ...
  write(2, new TextEncoder().encode(`tac: ${error instanceof Error ? error.message : String(error)}\n`))
  exit(1)
}
