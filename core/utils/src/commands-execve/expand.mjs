...
function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '-t' || arg === '--tabs') {
      ...
      write(2, new TextEncoder().encode(`expand: invalid option -- '${arg.slice(1)}'\n`))
      write(2, new TextEncoder().encode("Try 'expand --help' for more information.\n"))
      return 1
    }
  }
  ...
  if (files.length === 0) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        write(2, new TextEncoder().encode(`expand: ${file}: ${message}\n`))
        hasError = true
      }
    }
  }
...
try {
  ...
  write(2, new TextEncoder().encode(`expand: ${error instanceof Error ? error.message : String(error)}\n`))
  exit(1)
}
