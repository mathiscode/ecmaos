...
function main() {
  ...
  for (let i = 0; i < argsIn.length; i++) {
    ...
    if (arg === '-s' || arg === '--separator') {
      ...
      else { writeAll(2, new TextEncoder().encode("seq: option requires an argument -- 's'\n")); return 1 }
  ...
  if (args.length === 0) {
    writeAll(2, new TextEncoder().encode('seq: missing operand\n'))
    return 1
  }
  ...
  if (args.length === 1) {
    ...
    if (isNaN(last)) { writeAll(2, new TextEncoder().encode(`seq: invalid number: ${args[0]}\n`)); return 1 }
  } else if (args.length === 2) {
    first = parseFloat(args[0])
    last = parseFloat(args[1])
    if (isNaN(first) || isNaN(last)) { writeAll(2, new TextEncoder().encode('seq: invalid number\n')); return 1 }
  } else if (args.length === 3) {
    first = parseFloat(args[0])
    increment = parseFloat(args[1])
    last = parseFloat(args[2])
    if (isNaN(first) || isNaN(increment) || isNaN(last)) { writeAll(2, new TextEncoder().encode('seq: invalid number\n')); return 1 }
  } else {
    writeAll(2, new TextEncoder().encode('seq: too many arguments\n'))
    return 1
  }
  ...
  if (increment > 0) {
    ...
    writeAll(2, new TextEncoder().encode('seq: zero increment\n'))
    return 1
  }

  const output = numbers.map(n => Number.isInteger(n) ? n.toString() : n.toFixed(10).replace(/\.?0+$/, '')).join(separator)
  writeAll(1, new TextEncoder().encode(output + (separator === '\n' ? '' : '\n')))
...
try {
  ...
  writeAll(2, new TextEncoder().encode(`seq: ${error instanceof Error ? error.message : String(error)}\n`))
  exit(1)
}
