...
function main() {
  ...
  for (const arg of args) {
    if (!arg.startsWith('-')) {
      ...
      writeAll(2, new TextEncoder().encode(`rev: invalid option -- '${arg.slice(1)}'\n`))
      writeAll(2, new TextEncoder().encode("Try 'rev --help' for more information.\n"))
      return 1
    }
  }
  ...
  if (files.length === 0) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        writeAll(2, new TextEncoder().encode(`rev: ${file}: ${message}\n`))
        hasError = true
      }
    }
  }
...
try {
  ...
  writeAll(2, new TextEncoder().encode(`rev: ${error instanceof Error ? error.message : String(error)}\n`))
  exit(1)
}
